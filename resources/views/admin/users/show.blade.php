...
<div class="card p-4 mb-4">
    <div class="row align-items-start">
        <div class="col-12 col-md-9 d-flex gap-3">
            ...
            <div class="min-w-0 flex-grow-1">
                ...
                <h2 class="h4 text-primary mb-1">{{ ucwords($user->name) }}</h2>
                ...
                <div class="row g-3">
                    <div class="col-6 col-lg-4">
                        ...
                        <p class="text-light mb-0"><i class="bi bi-telephone text-body-secondary me-1"></i>{{ $user->phone ?? '—' }}</p>
                    </div>
                    <div class="col-6 col-lg-4">
                        <p class="stat-label mb-1">Country</p>
                        <p class="text-light mb-0"><i class="bi bi-geo-alt text-body-secondary me-1"></i>{{ $user->country ?? '—' }}</p>
                    </div>
                    <div class="col-6 col-lg-4">
                        <p class="stat-label mb-1">Registered</p>
                        <p class="text-light mb-0"><i class="bi bi-calendar3 text-body-secondary me-1"></i>{{ $user->created_at->format('M j, Y') }}</p>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="row mt-4 pt-4 border-top border-secondary-subtle">
        ...
        <div class="col-6 col-md-3 mt-3">
            ...
            <p class="text-light mb-0">{{ $user->referral_code ?? '—' }}</p>
        </div>
    </div>
</div>
...
<div class="row g-3">
    <div class="col-6 col-md-3">
        <a href="{{ route('admin.deposits', ['user' => $user->id]) }}" class="stat-card d-flex align-items-center gap-3 text-decoration-none">
            <span class="stat-icon stat-icon-success"><i class="bi bi-box-arrow-in-down"></i></span>
            <div class="min-w-0">
                <p class="stat-label mb-1">Deposits</p>
                <p class="stat-value mb-0">{{ $user->deposits->count() }}</p>
            </div>
        </a>
    </div>
    <div class="col-6 col-md-3">
        <a href="{{ route('admin.withdrawals', ['user' => $user->id]) }}" class="stat-card d-flex align-items-center gap-3 text-decoration-none">
            <span class="stat-icon stat-icon-danger"><i class="bi bi-box-arrow-up"></i></span>
            <div class="min-w-0">
                <p class="stat-label mb-1">Withdrawals</p>
                <p class="stat-value mb-0">{{ $user->withdrawals->count() }}</p>
            </div>
        </a>
    </div>
    <div class="col-6 col-md-3">
        <a href="{{ route('admin.loans', ['user' => $user->id]) }}" class="stat-card d-flex align-items-center gap-3 text-decoration-none">
            <span class="stat-icon stat-icon-info"><i class="bi bi-cash-coin"></i></span>
            <div class="min-w-0">
                <p class="stat-label mb-1">Loans</p>
                <p class="stat-value mb-0">{{ $user->loans->count() }}</p>
            </div>
        </a>
    </div>
    <div class="col-6 col-md-3">
        <a href="{{ route('admin.investments', ['user' => $user->id]) }}" class="stat-card d-flex align-items-center gap-3 text-decoration-none">
            <span class="stat-icon stat-icon-primary"><i class="bi bi-graph-up-arrow"></i></span>
            <div class="min-w-0">
                <p class="stat-label mb-1">Investments</p>
                <p class="stat-value mb-0">{{ $user->investments->count() }}</p>
            </div>
        </a>
    </div>
    <div class="col-6 col-md-3">
        <a href="{{ route('admin.profits', ['user' => $user->id]) }}" class="stat-card d-flex align-items-center gap-3 text-decoration-none">
            <span class="stat-icon stat-icon-success"><i class="bi bi-graph-up"></i></span>
            <div class="min-w-0">
                <p class="stat-label mb-1">Profits</p>
                <p class="stat-value mb-0">{{ $user->profits->count() }}</p>
            </div>
        </a>
    </div>
    <div class="col-6 col-md-3">
        <a href="{{ route('admin.bonuses', ['user' => $user->id]) }}" class="stat-card d-flex align-items-center gap-3 text-decoration-none">
            <span class="stat-icon stat-icon-info"><i class="bi bi-gift"></i></span>
            <div class="min-w-0">
                <p class="stat-label mb-1">Bonuses</p>
